/**
 * Created by михайло on 28.05.2017.
 */
...
function repostGetEvent(d) {
    $("#repostEvent").empty()
    $.ajax({
        url:'/event/'+d,
        method:'GET',
        contentType:'application/json',
        success:function (event) {if (event!==null){
            $("#repostEvent").append("")
...
function repostEvent(id) {
    $.ajax({
        url: '/event/add/repost/'+id,
        method: 'POST',
        contentType: 'application/json',
        data: JSON.stringify(getRepost()),
        success: function () {
